...
func show_results(correct: int, _wrong_items: Array) -> void:
	...
	if correct >= 12: # Passing score
		...
		# ✅ Quest progression
		if QuestManager.has_quest("finish_book2") and not QuestManager.is_quest_completed("finish_book2"):
			QuestManager.complete_quest("finish_book2")

			var next_quest = QuestManager.get_quest_template("after_book2")
			if next_quest:
