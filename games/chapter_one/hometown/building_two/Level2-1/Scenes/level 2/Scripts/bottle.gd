...
func _ready():
	...
	add_to_group("BottleGroup")

	print("🍾 Bottle %s → M:%.1f | V:%.1f | ρ:%.2f" % [bottle_id, object_mass, volume, density])
...
func _update_data_label():
	...
	if not ChallengeManager.initialized:
		print("⚠️ ChallengeManager not initialized.")
		return

	data_label.text = "Mass: %.2f kg\nVolume: %.2f L" % [object_mass, volume]
	print("🧾 %Data updated →", data_label.text)
