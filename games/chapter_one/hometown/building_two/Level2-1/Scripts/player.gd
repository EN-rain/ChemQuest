...
func pickup_object() -> void:
	if is_in_range and Input.is_action_just_pressed("pickup") and not held_object:
		...
		if held_object.has_method("_on_picked_up"):
			held_object._on_picked_up()
		else:
			push_warning("%s has no _on_picked_up()!" % held_object.name)
		...
		var density = held_object.get_density() if held_object.has_method("get_density") else 1.0
